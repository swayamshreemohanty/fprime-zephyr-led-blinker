...
void UartBufferAdapter::bufferIn_handler(FwIndexType portNum, Fw::Buffer& fwBuffer) {
    ...
    Fw::Logger::log("[UartBufferAdapter] bufferIn_handler: portNum=%d bufferSize=%d ready=%d\n", 
                   portNum, fwBuffer.getSize(), m_driverIsReady);
    
    if (m_driverIsReady) {
        Fw::Logger::log("[UartBufferAdapter] TX -> UART: ");
        for (FwSizeType i = 0; i < (fwBuffer.getSize() > 32 ? 32 : fwBuffer.getSize()); i++) {
            Fw::Logger::log("%02x ", fwBuffer.getData()[i]);
        }
        Fw::Logger::log("\n");
        ...
        if (status != Drv::ByteStreamStatus::OP_OK) {
            ...
            Fw::Logger::log("[UartBufferAdapter] UART TX SUCCESS\n");
        }
    } else {
